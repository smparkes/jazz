...
There are examples in <tt>examples/*</tt> and <tt>spec/examples/*</t>. You can run the
...
Running <tt>jazrb.watchr</tt> (either with <tt>watchr
jazrb.watchr</tt> or even <tt>./jazrb.watchr</tt> will run all the
tests and then watch the file system for changes. If it sees a change,
...
<tt>spec/example/hello_world_spec.js</tt>. And example of the latter
...
the only change necessary.)  Note that to run the tests from files in
Firefox, you'll need to set the
<tt>security.fileuri.strict_origin_policy</tt> option to true.
