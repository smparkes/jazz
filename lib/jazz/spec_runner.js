...
(function(){return this;}()).jazz = (function($){
  ...
  var debug = global.debug || global.console && global.console.debug;
  ...
  var find_specs = function() {
    ...
    path = construct_path( path + "/" + jazz.app_root, jazz.app_root + "/spec/" );
  ...
  var jazz_path = "JAZZ_JS_PATH";

  var run_specs = function(){
    var run = !!global.Envjs && global.Ruby.ENV[jazz_path];
    if(!global.jazz.root) {
      return false;
    }
    if ( !run ) {
  ...
  if( run_specs() ){
    ...
    if(global.jasmine && (!global.Envjs || !global.Ruby.ENV[jazz_path])) {
      ...
      if(window.Envjs && jasmine.EnvjsReporter && global.Ruby.ENV[jazz_path]){
